...
public class TGregRegistry {
	...
	public void addToolMaterial(int matID, Materials m) {
		TConstructRegistry.addToolMaterial(matID, m.getName(), m.mLocalizedName, m.mToolQuality,
	...
	public float getMultiplier(Materials m, String key) {
		return (float) TGregworks.config.get(Config.onMaterial(key), m.getName(), 1.0, null, 0, 10000).getDouble(1.0);
	}
	...
	public float getStoneboundLevel(Materials m) {
		return (float) TGregworks.config.get(Config.StoneboundLevel, m.getName(),
	...
	public int getReinforcedLevel(Materials m) {
		return TGregworks.config.get(Config.ReinforcedLevel, m.getName(),
	...
	private boolean doesMaterialExist(Materials m) {
		return !addMaterialsAnyway && TConstructRegistry.toolMaterialStrings.containsKey(m.getName());
